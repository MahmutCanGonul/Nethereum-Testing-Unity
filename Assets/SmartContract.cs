...
public class SmartContract : MonoBehaviour
{
    ...
    private IEnumerator DeployContract(string ownerAddress, string ownerPrivateKey, string tokenName, string tokenSymbol, BigInteger balanceToken, BigInteger ether,BigInteger tip)
    {
        ...
        {
            Debug.Log("Invalid address or private key!");
            yield break;
        }
        ...
        var transactionReceiptPolling = new TransactionReceiptPollingRequest(TestNethereum.url);

        //checking every 2 seconds for the receipt
        yield return transactionReceiptPolling.PollForReceipt(transactionRequest.Result, 2);

        if (transactionReceiptPolling.Exception != null)
        {
            Debug.Log(transactionReceiptPolling.Exception.Message);
            yield break;
        }
        ...
        BigInteger tip,string contractAddress)
    {
        ...
        {
            Debug.Log(transactionTransferRequest.Exception.Message);
            yield break;
        }

        var transactionTransferHash = transactionTransferRequest.Result;
        Debug.Log("Transfer txn hash:" + transactionTransferHash);
        var transactionReceiptPollingRequest = new TransactionReceiptPollingRequest(TestNethereum.url);
        yield return transactionReceiptPollingRequest.PollForReceipt(transactionTransferHash, 2);

        if (transactionReceiptPollingRequest.Exception != null)
        {
            Debug.Log(transactionReceiptPollingRequest.Exception.Message);
            yield break;
        }
